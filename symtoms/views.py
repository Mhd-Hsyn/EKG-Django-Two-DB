...
class Scripting(APIView):
    def get(self, request):
        disease_name = request.GET.get('disease_name')
        version =  request.GET.get('version')
        symtoms_data = symtoms.objects.filter(diseasesname__name = disease_name, diseasesname__question__version_list = version).order_by('-probability').values("name","probability",question_id = F('diseasesname__question__id'))
        # print("symtom_data",len(symtoms_data))

        # for symtom_data in symtoms_data:
        #     print("question_id",symtom_data["question_id"])
        #     print("name==>",symtom_data["name"])
        #     print("probability",symtom_data["probability"])
        #     print("----------------------------------------------------------------")

        return Response({"status":True,"data":symtoms_data})
